...
def prepare_data(verbose=True):
    ...
    # 在开始就计算维度信息
    dimensions, financial_columns = calculate_feature_dimensions(df)
    
    print("\n[Info] Feature Dimensions Summary:")
    print(f"- Total Feature Dimensions: {dimensions['total']}")
    if USE_FINANCIAL_INDICATORS:
        print(f"- Financial Indicator Dimensions: {dimensions['financial_indicators']}")
        if verbose:
            print("\nFinancial Indicators:")
            for col in financial_columns:
                print(f"  - {col}")
    if USE_TEXT_EMBEDDING:
        print(f"- Text Embedding Dimensions: {dimensions['text_embedding']}")
    print("\n")
    ...
    if USE_FINANCIAL_INDICATORS:
        print(f"[Info] - Financial indicator dimensions: {dimensions['financial_indicators']}")
    ...
    # 检查数据类型并打印调试信息
    print(f"[Debug] Data type check before reshape:")
    print(f"- all_features dtype: {all_features.dtype}")
    print(f"- all_features shape: {all_features.shape}")
    print(f"- First row sample: {all_features[0][:10]}")  # 打印第一行的前10个元素
    
    # 确保数据是数值类型
    if all_features.dtype == object:
        print("[Warning] Found non-numeric data, attempting to convert...")
        # 如果是文本数据，先进行必要的预处理
        if USE_TEXT_EMBEDDING:
            # 文本数据应该已经通过FinBERT转换为数值向量
            pass
        else:
            # 如果有其他非数值列，需要进行相应的处理
            numeric_features = []
            for i in range(all_features.shape[1]):
                col = all_features[:, i]
                try:
                    numeric_features.append(col.astype(np.float32))
                except ValueError as e:
                    print(f"[Error] Column {i} contains non-numeric data: {str(e)}")
                    print(f"Sample data from column {i}: {col[:5]}")
                    raise ValueError(f"Column {i} contains non-numeric data that cannot be converted to float")
            all_features = np.column_stack(numeric_features)
...
def calculate_feature_dimensions(df):
    """
    计算特征维度
    """
    financial_columns = [col for col in df.columns if col not in EXCLUDE_COLUMNS]
    dimensions = {
        'financial_indicators': len(financial_columns),
        'text_embedding': 100 if USE_TEXT_EMBEDDING else 0,
        'total': len(financial_columns) + (100 if USE_TEXT_EMBEDDING else 0)
    }
    return dimensions, financial_columns